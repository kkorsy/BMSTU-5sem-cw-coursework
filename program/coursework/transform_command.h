...
{
public:
    RotateCamera(const Point& cent, const Vector& vect);
...
class RotatePetal : public RotateCommand
{
public:
    RotatePetal(const Point& center, const Vector& vect);
    virtual ~RotatePetal();

    virtual void execute(weak_ptr<Scene> scene, weak_ptr<Petal> p);
};
