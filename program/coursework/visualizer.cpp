...
Visualizer::Visualizer(shared_ptr<Drawer>& drawer) :
    _draw(drawer) {}

Visualizer::~Visualizer() {}


void Visualizer::set_draw(const shared_ptr<Drawer> &drawer)
{
    _draw = drawer;
}
...
void Visualizer::set_light(const LightSource &light)
{
    ...
    Point pnt = light.get_position();
    Vector vect(pnt.x, pnt.y, pnt.z);
    ...
    if (abs(pnt.z) > 1e-5)
        beta = atan(pnt.x / pnt.z);
    else
        beta = copysign(PI / 2, pnt.x);
    pnt.rotate_oy(Point(0, 0, 0), beta);

    if (abs(pnt.z) > 1e-5)
        alpha = -atan(pnt.y / pnt.z);
    else
        alpha = -copysign(PI / 2, pnt.y);
    if (pnt.z < -1e-5)
        alpha += PI;
...
void Visualizer::draw_model(const Model &model)
{
    ...
    for (auto side : model.s_arr)
    {
        ...
        while (p_side.active_edges.size() && p_side.y_temp > min_y)
        {
            ...
            for (; pnt.x < st; pnt.x++)
            {
                //_draw->draw_point(pnt, side->color, i, invt_pnt1);
...
void Visualizer::draw_shadows(const Model &model)
{
    int max_y = _draw->get_max_y();
    int min_y = _draw->get_min_y();
    int min_x = _draw->get_min_x();
    ...
    for (auto side : model.s_arr)
    {
        ...
        _project_side_shd(p_side, side->vertex_arr);
...
double Visualizer::_glare_i(const Point& pnt, const Vector& n)
{
    ...
    Vector res((vct1.x + vct2.x) / 2, (vct1.y + vct2.y) / 2, (vct1.z + vct2.z) / 2);
...
void Visualizer::clear(int night)
{
    _draw->fill_zmap_onedepth(-10000);

    QColor bg = _draw->get_background();
    if (night == 1)
    {
        bg.setRed(bg.red() - 68 / 60);
        bg.setGreen(bg.green() - 148 / 60);
        bg.setBlue(bg.blue() - 147 / 60);
    }
    else if (night == -1)
    {
        bg.setRed(bg.red() + 68 / 60);
    ...
    _draw->set_background(bg.rgb());

    _draw->make_map_plain();
...
Point Visualizer::_project_point(const Point &pnt)
{
    ...
    k = 500 / sqrt(camera.z * camera.z + 0.001);
...
Point Visualizer::_project_point_shd(const Point &pnt)
{
    Point result = pnt;
    Point light = _light.get_position();
    double k;

    result.rotate(light, _light_dir);
    ...
    k = 2;
    result.x = (result.x - light.x) * k;
    result.y = (result.y - light.y) * k;
...
Point Visualizer::_invert_project_point(const Point &pnt)
{
    ...
    k = 500 / sqrt(camera.x * camera.x + camera.z * camera.z + 0.001);
...
void Visualizer::_project_side_shd(ProjectedSide &side, const vector<shared_ptr<Vertex>>& vertex_arr)
{
    for (size_t i = 0; i < vertex_arr.size(); i++)
    {
        Point pnt1 = _project_point_shd(*vertex_arr[i]);
        Point pnt2 = _project_point_shd(*vertex_arr[(i + 1) % vertex_arr.size()]);
