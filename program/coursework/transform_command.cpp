...
RotateCamera::RotateCamera(const Point& cent, const Vector& vect) :
    RotateCommand(cent, vect) {}
...
RotatePetal::RotatePetal(const Point& center, const Vector& vect) :
    RotateCommand(center, vect) {}

RotatePetal::~RotatePetal() {}

void RotatePetal::execute(weak_ptr<Scene> scene, weak_ptr<Petal> p)
{
    TransformManager(scene, p, _transf).petal_execute();
}
